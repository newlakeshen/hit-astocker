...
class TierProfitEffect:
    ...
    is_small_sample: bool = False  # prev_count < 5, 下游应考虑 fallback
...
class ProfitEffectSnapshot:
    ...
    def tier_for_height_by_type(self, height: int, is_20cm: bool) -> TierProfitEffect | None:
        ...
        小样本 fallback: 如果该层样本不足 (is_small_sample=True),
        自动降级到低一层 (e.g. 3板→2板→首板). 直到找到样本充足的层,
        或降到首板仍然小样本则返回原始结果.
        """
        tier_name = _height_to_tier(height)
        ...
        if result is None:
            # 该层无数据, fallback 到总体层
            return self.tier_for_height(height)
        if result.is_small_sample and height > 1:
            # 小样本 fallback: 递归查低一层
            fallback = self.tier_for_height_by_type(height - 1, is_20cm)
