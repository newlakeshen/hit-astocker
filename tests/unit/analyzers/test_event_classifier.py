...
def _init_calendar(in_memory_db):
    in_memory_db.executemany(
        ...
        [
            ("20260303", 1), ("20260304", 1), ("20260305", 1),
            ("20260306", 1), ("20260307", 0), ("20260308", 0),
        ],
    )
    in_memory_db.commit()
...
def _make_kpl_record(ts_code, name, lu_desc="", theme="", tag="涨停"):
    return {
        "trade_date": "20260306", "ts_code": ts_code, "name": name,
        "lu_time": "09:35", "ld_time": "", "lu_desc": lu_desc, "tag": tag,
        "theme": theme, "net_change": 0, "bid_amount": 0, "status": "封板",
        "pct_chg": 10.0, "amount": 50000, "turnover_rate": 5.0,
        "lu_limit_order": 0,
    }
...
def test_layer2_concept_classification(in_memory_db):
    ...
    repo.upsert_many([{
        "id": "C001", "concept_name": "碳中和概念",
        "ts_code": "000002.SZ", "name": "测试B",
        "in_date": "20200101", "out_date": "",
    }])
    in_memory_db.commit()
...
def test_layer_priority_announcement_wins(in_memory_db):
    ...
    ann_repo.upsert_many([{
        "ann_date": "20260306", "ts_code": "000005.SZ",
        "title": "业绩预增200%", "ann_type": "业绩预告", "content": "",
    }])
    concept_repo = BaseRepository(in_memory_db, "concept_detail")
    concept_repo.upsert_many([{
        "id": "C002", "concept_name": "新能源车",
        "ts_code": "000005.SZ", "name": "优先级测试",
        "in_date": "20200101", "out_date": "",
    }])
    in_memory_db.commit()
...
def test_diffusion_rate(in_memory_db):
    ...
    concept_repo = BaseRepository(in_memory_db, "concept_detail")
    concept_repo.upsert_many([
        {"id": "C100", "concept_name": "碳中和概念", "ts_code": "000001.SZ", "name": "A", "in_date": "", "out_date": ""},
        {"id": "C100", "concept_name": "碳中和概念", "ts_code": "000002.SZ", "name": "B", "in_date": "", "out_date": ""},
        {"id": "C100", "concept_name": "碳中和概念", "ts_code": "000003.SZ", "name": "C", "in_date": "", "out_date": ""},
        {"id": "C100", "concept_name": "碳中和概念", "ts_code": "000004.SZ", "name": "D", "in_date": "", "out_date": ""},
    ])
    in_memory_db.commit()
